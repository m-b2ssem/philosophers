...
/*   Updated: 2024/03/13 01:27:45 by bmahdi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_philo_status(t_philo *philo)
{
	...
	mutex_lock_and_unlock(&philo->leads->checker, 'u');
	mutex_lock_and_unlock(&philo->reaper, 'l');
    return (0);
}

int check_philo_status2(t_philo *philo)
{
    mutex_lock_and_unlock(&philo->leads->checker, 'l');
    if (philo->leads->died == 1 || philo->leads->all_ate == philo->leads->philos_num)
    {
        mutex_lock_and_unlock(&philo->leads->checker, 'u');
        return (1);
    }
...
{
    mutex_lock_and_unlock(ph->l_fork, 'l');
    ft_message("has taken a fork\n", ph);
    mutex_lock_and_unlock(ph->l_fork, 'u');
    return (1);
}
